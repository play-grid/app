...
export class GameSessionManager {
  ...
  dispatchAction(action: any): void {
    ...
    const coreActionTypes = GameActionSchema.options.map(schema => schema.shape.type.def.values);
    ...
    try {
      ...
      if (coreActions.has(action?.type)) {
        ...
        const gamedef = this.gameDefinition.actionSchema;
        validatedAction = action as z.infer<typeof gamedef>;
      }
      else {

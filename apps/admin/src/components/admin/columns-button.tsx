import type { ExtractRecordPaths, HintedString, Identifier, RaRecord, SortPayload } from 'ra-core';
  ...
  return createPortal(
    <ul className="max-h-[50vh] p-1 overflow-auto">
      ...
      {paddedColumnRanks.map((position, index) => (
        <DataTableColumnRankContext value={position} key={index}>
          <DataTableColumnFilterContext
            value={columnFilter}
            key={index}
          >
...
function padRanks(ranks: number[], length: number) {
  return ranks.concat(
    Array.from({ length: length - ranks.length }, (_, i) => ranks.length + i),
  );
}

function fieldLabelMatchesFilter(fieldLabel: string, columnFilter?: string) {
  return columnFilter
    ? diacritic
        .clean(fieldLabel)
        .toLowerCase()

 ...
 */
export function useSupportCreateSuggestion<T = unknown,>(options: SupportCreateSuggestionOptions<T>): UseSupportCreateValue<T> {
  const {
    create,
 ...
 */
export function useCreateSuggestionContext() {
  const context = use(CreateSuggestionContext);
  if (!context) {
    throw new Error(
